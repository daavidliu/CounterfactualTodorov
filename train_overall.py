...
JUDGE_NAME = "Unbabel/M-Prometheus-14B"

POLICY_PATH = f"{LLMS_PATH}/{POLICY_NAME}"
JUDGE_PATH = f"{LLMS_PATH}/{JUDGE_NAME}"

TIMESTAMP = time.strftime("%H%M")
OUTPUT_DIR = os.path.join(CUR_DIR, f"{POLICY_NAME.replace('/', '-')}-overall-{TIMESTAMP}")
...
def extract_score(text):
    """
    Parses the judge output to extract the numeric score.
    Dynamically adjusts detection range based on the rubric (Max 3 or 5).
    """
    # 1. SET THE MAX SCORE DYNAMICALLY
    # If 'narrativity' is in your rubrics list, use 5, otherwise use 3
    ...
    # 4. DYNAMIC FALLBACK REGEX
    # Notice the 'rf' prefix: 'r' for regex raw string, 'f' for f-string formatting
    # This creates r'\b([1-5])\b' or r'\b([1-3])\b' automatically.
...
def reward_function(prompts, completions, **kwargs):
    ...
    Applies the judge rubric and calculates basic length penalties.
    """
    all_inputs = []
    meta_map = [] 
    ...
    # --- B. INFERENCE (JUDGE) ---
    # Sort for efficient batching (padding reduction)
    ...
    for flat_idx, score in raw_scores.items():
        ...
        # 2. Length Penalty Calculation
        original_len = len(original_data['original_ending'])
        new_len = len(completions[comp_idx])
        
        penalty = 0.0
        if new_len < (0.5 * original_len):
            penalty = 0.5
        elif new_len > (3.0 * original_len):
            penalty = 0.5
            
        final_reward_val = max(current_reward - penalty, 0.1)
...
class RewardEarlyStoppingCallback(TrainerCallback):
    def __init__(self, threshold=2.95, patience=10):
...
if __name__ == "__main__":
    ...
    trainer = GRPOTrainer(
        ...
        callbacks=[
            RewardEarlyStoppingCallback(threshold=2.9, patience=20),
